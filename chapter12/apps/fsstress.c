...
#define CHURN_FILES 12
#define FILL_FILES 24
...
void main(void) {
    ...
    for (int i = 0; i < FILL_FILES; i++) {
        ...
        for (int blk = 0; blk < 3; blk++) {
            int n = user_write(g, blk * BLOCK_SIZE, buf, BLOCK_SIZE);
            if (n != BLOCK_SIZE) {
                ok = 0;
                break;
            }
    ...
    printf("fsstress: created %d moderate fill files\n", created);
    check(created >= 4, "disk accepted several moderate fill files");
    ...
    if (failures == 0) printf("fsstress: all checks passed\n");
    else printf("fsstress: %d checks failed\n", failures);
}
